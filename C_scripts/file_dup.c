/************************************************************
  ...
  The program copies the directory structure of the file
  specified in FILE and rewrite to DESTFILE
...
#endif
{
    ...
    hid_t       d_file, d_group, s_file, s_group;
    ...
    switch (infobuf.type) {
        case H5O_TYPE_GROUP:
            ...
            // d_file = H5Fcreate(DESTFILE, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
        ...
        case H5O_TYPE_DATASET:
            ...
            status = H5Gclose(d_group);
            // status = H5Gclose(s_group);
            status = H5Fclose(d_file);
            // status = H5Fclose(s_file);
